...
const subcatids_of_catids_arr = [
  ...
  function asj(params, subcatids_of_catids_arr) {

    const allSubCatIds = subcatids_of_catids_arr.flatMap(category => category.sub_cat_ids);    
    let arr = []
    asyncLoop(
        allSubCatIds,
        async(sub_category_id, next)=>{
                    try {
                        params.sub_category_id = sub_category_id;
                        let res = await home.service_provider_list_es(params);
                        arr.push(res);
                        next(); 
                    } catch (error) {
                        console.log(error);
